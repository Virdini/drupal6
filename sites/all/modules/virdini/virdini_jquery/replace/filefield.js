...
Drupal.behaviors.filefieldPreviewLinks = function(context) {
  $('div.filefield-element div.widget-preview a', context).click(Drupal.filefield.openInNewWindow).prop('target', '_blank');
}
...
Drupal.filefield = {
  ...
  disableFields: function(event) {
    ...
    setTimeout(function(){
      $disabledFields.removeProp('disabled');
    }, 1000);
  },
  progressBar: function(event) {
    ...
    if ($progressId.size()) {
      var originalName = $progressId.prop('name');

      // Replace the name with the required identifier.
      $progressId.prop('name', originalName.match(/APC_UPLOAD_PROGRESS|UPLOAD_IDENTIFIER/)[0]);

      // Restore the original name after the upload begins.
      setTimeout(function() {
        $progressId.prop('name', originalName);
      }, 1000);
    }
